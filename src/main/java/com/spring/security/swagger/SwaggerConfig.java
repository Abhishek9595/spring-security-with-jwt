package com.spring.security.swagger;
...
public class SwaggerConfig {
	...
	@Autowired
	private SwaggerProperties property;

	@Bean
	public Docket api() {
		return new Docket(DocumentationType.SWAGGER_2).apiInfo(apiInfo())
				.securityContexts(Arrays.asList(securityContext())).securitySchemes(Arrays.asList(apiKey())).select()
				.apis(RequestHandlerSelectors.basePackage("com.spring.security")).paths(PathSelectors.any()).build();
	}
